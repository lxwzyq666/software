...
class BERTTrainer:
    ...
    def __init__(self, bert: BERT, vocab_size: int,
        ...
        # self.criterion = nn.NLLLoss(ignore_index=0): 这行代码定义了一个负对数似然损失函数
        # （Negative Log Likelihood Loss），并将其赋值给了 self.criterion。
        # 负对数似然损失函数通常用于多分类问题，它将模型预测的概率分布与实际标签的对数概率进行比较，
        # 并计算损失。ignore_index=0 表示在计算损失时忽略标签为 0 的部分，通常用于处理填充或者无效的标签。
        #self.time_criterion = nn.MSELoss(): 这行代码定义了一个均方误差损失函数
        # （Mean Squared Error Loss），并将其赋值给了 self.time_criterion。均方误差损失函数通常用于回归问题，它计算模型预测值与实际值之间的平方差，并求取平均值作为损失。
        #   self.hyper_criterion = nn.MSELoss():
        # 这行代码也是定义了一个均方误差损失函数，并将其赋值给了 self.hyper_criterion。这
        # 个损失函数可能用于另外一个任务或者子模型，用于计算模型的超球损失（hypersphere loss）。
        ...
        self.radius = 0   # 初始化半径
        self.hyper_center = None   # 初始化超球中心
        self.nu = 0.25 #这行代码设置了 deep SVDD 模型中的超参数 nu 的值为 0.25。nu 是一个超参数，控制正常样本在超球内部的比例。
                    # 通常，它取一个小于 0.5 的值，表示正常样本在超球内部的比例，这也被称为支持向量比例。
